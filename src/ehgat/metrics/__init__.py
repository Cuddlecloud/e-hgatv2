"""Module 5 quality indicators: hypervolume, GD+/IGD+, and spread.

All indicators are **minimisation-oriented**, depend only on objective vectors (no Torch),
and operate on the exact golden Pareto front ``PF*`` so the effectiveness claims (H1, H2)
are absolute rather than relative. They are cross-validated against ``pymoo`` in the test
suite.
"""
