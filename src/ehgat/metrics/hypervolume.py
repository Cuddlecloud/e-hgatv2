...
reference point** ``r``. For minimisation ``r`` must be weakly dominated by every point
(i.e. no smaller than the worst values), so a *larger* HV means a front that is both
closer to the true front and better spread -- the single number used for the H1
convergence curve.

The bi-objective case admits an exact ``O(k log k)`` sweep, so no Monte-Carlo estimate is
needed. For a fair comparison across methods the **same** reference point must be used;
:func:`nadir_reference` derives it from the nadir of one or more fronts (typically the
