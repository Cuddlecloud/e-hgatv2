"""Canonical schedule decoder: 4N random-key chromosome -> :class:`Schedule`.
...
chromosome; deadlocks are only possible after the targeted mutations of Module 4,
which is exactly why the evaluator re-validates acyclicity with Kahn's algorithm).
"""
...
def decode(keys: np.ndarray, instance: Instance) -> Schedule:
    """Decode a ``4N`` random-key chromosome into a :class:`Schedule`."""
