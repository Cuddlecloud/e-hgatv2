...
The paper's instances are published only as dense tables (Table 5 task sets + Table 4
QC<->LU distance matrix). This module turns a **transcribed JSON** of those task sets into
validated :class:`~ehgat.environment.instance.Instance` objects so the surrogate / search /
explanation experiments run on the *real* published instances instead of synthetic toys.
...
- **coupled** (``peak_power=<kW>``) — the author's peak-power extension applied *on top of*
  the real instance (real geometry + real tasks + the coupling).
...
def _synthesize_distance(nodes: list[str]) -> DistanceMatrix:
    """Deterministic asymmetric grid geometry for nodes outside the real Table 4 matrix.

    QCs and LUs are laid on two parallel rows; distance = Manhattan-style with a small
    directional asymmetry so empty/loaded legs differ (mirrors the real matrix's character).
    Used ONLY when the real matrix lacks a node (large DL instances) and explicitly opted in.
    """
...
def load_dsdl_instance(
    ...
    *,
    ...
    """Build one :class:`Instance` from a transcribed DS/DL ``record`` (see module docstring)."""
    ...
    if not needed.issubset(known):
        ...
        if not allow_synthetic_geometry:
            raise ValueError(
                ...
                f"(the real Table 4 only covers QC1..QC6/LU1..LU6). Pass an extended "
...
def _distance_from_table4(table4: dict) -> DistanceMatrix:
    """Build a :class:`DistanceMatrix` from the ``table4_distance_matrix_m`` block."""
...
def load_tables_4_5(
    ...
    *,
    ...
    """Load the real ``data/tables_4_5.json`` (Homayouni & Fontes 2022 book chapter).

    This is the **loading / single-cycle** small-instance set (``L01..L35``): Table 4 is
    the real QC<->LU distance matrix and Table 5 gives, per QC row, an ordered list of
