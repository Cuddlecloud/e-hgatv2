"""GNN/TAPE-derived feature-importance landscape (Req 2) -- model-native and scalable.
...
Why this scales where Sobol does not: every score is **one model forward/backward pass per
schedule** (O(1) in the number of decision families, batchable on GPU), whereas grouped
Sobol costs ``base_samples * (2 + families)`` *exact-evaluator* calls and the critical-path
cascade costs ``O(N * samples)`` exact re-evaluations -- both intractable at the DL
instance sizes (40-160 containers). Here the cost is flat in N per schedule.

The decision families mirror the professor's named variables and the random-key blocks:
``sequence`` (task order / QC precedence), ``assignment`` (which AGV), ``empty_speed`` and
``loaded_speed`` (per-leg travel speed).

**Validation, not Sobol.** The *same* aggregator runs on the exact TAPE oracle
(:func:`~ehgat.explain.tape_explainer.explain_schedule`) at small N, giving a ground-truth
landscape the GNN landscape is checked against (:func:`landscape_rank_agreement`). At large
N only the GNN path is feasible -- that is precisely the amortisation the surrogate buys.
"""
...
# The professor's four decision-variable families (random-key chromosome blocks).
...
def aggregate_landscape(
    ...
    *,
    ...
    ``explanations`` may come from the fused model (:func:`gnn_landscape`) or the exact
    oracle (:func:`exact_landscape`); the aggregation is identical, which is what makes the
...
def exact_landscape(instance: Instance, schedules: list[Schedule]) -> GnnLandscapeResult:
    ...
    the validation reference for :func:`gnn_landscape`, not a scalable path.
    """
