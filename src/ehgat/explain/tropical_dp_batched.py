...
The per-graph :class:`~ehgat.explain.tropical_dp.TropicalMaxPlus` runs a Python loop over
...
per-layer node/edge groupings) is graph-structure-only, so it is built **once** and reused for
all epochs (the leg-time/wait *values* change; the topology does not).
