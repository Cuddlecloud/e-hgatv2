"""Semantic-tensor assertions (governing Directive Section I).

In a physically grounded GNN, tensor dimensions can align numerically while
cross-contaminating physical constraints (e.g. multiplying ``Empty_Energy`` data with
weights reserved for ``Travel_Time``). PyTorch will not crash, but the structural
attention heuristic collapses into noise.
...
so they can be unit-tested against NumPy arrays without importing Torch.
"""
