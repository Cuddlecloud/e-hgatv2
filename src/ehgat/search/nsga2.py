...
These are the shared building blocks for both the BRKGA baseline (Module 2) and the
attention-guided search (Module 4): **fast non-dominated sorting** partitions a set of
...
def crowding_distance(objectives: Sequence[Objective], front: Sequence[int]) -> dict[int, float]:
    ...
    Distances are summed over objectives, each normalised by that objective's range
    across the front, so they are comparable across objectives of different scales.
    """
