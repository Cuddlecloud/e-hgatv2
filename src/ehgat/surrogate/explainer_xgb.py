"""XGBoost + TreeSHAP baseline surrogate (Paper-3 style, Torch-free).
...
-- and attributes predictions with **TreeSHAP** (exact for trees), exactly the
sequencing + speed attribution Paper 3 uses to guide its localized search.

It depends only on NumPy / XGBoost / SHAP (no Torch / PyG), so it can run independently
of the GNN stack while sharing the same random-schedule sampling protocol.
"""
