...
- **Cross-relation fusion uses semantic attention** (HAN-style): a task's completion is
  ...
  and QC-chain messages selects *which resource gates the task*. This matters because in
  a *resolved* schedule each task has a single predecessor per resource, so the
  per-relation GATv2 softmax is degenerate (always 1); the informative, non-degenerate
  attention is this cross-resource one.
- **Edge attention** ``alpha`` is extracted (detached) per arc as the destination's
  ...
  ``agv`` arc is the bottleneck the Module-4 mutation targets; its faithfulness is later
  validated against the Oracle's exact critical path (H3).
...
class _HeteroSemanticAttnLayer(nn.Module):
    ...
    In a resolved schedule every task has at most one AGV and one QC predecessor, so the
    per-relation GATv2 softmax over a single incoming arc is degenerate (always 1). The
    ...
    exposed as the **criticality of that incoming arc** (used by the Module-4 mutation).
    """
...
class EHGATv2(nn.Module):
    ...
    def __init__(self, config: EHGATv2Config | None = None) -> None:
        ...
        # be fed at inference (Module 4) while training stays well-conditioned. Features
    ...
    def forward(
        ...
        ``data`` may be a single :class:`HeteroData` or a PyG mini-batch of them.
        """
