"""Landscape analysis (Module 6): exact, cascade-aware feature-importance for Req 2.

The headline evidence for the **landscape / feature-importance** requirement: an algorithm
...
- :func:`critical_path_landscape` -- exact critical-path attribution: per-task AGV/QC
...
- :func:`pareto_contrast` -- *why Pareto-optimal*: which decision descriptors separate the
  non-dominated set from the dominated set (Cliff's delta effect size).
- :func:`tabular_failure_boundary` -- the **Claim 3** foil: where TreeSHAP (a fitted,
...
# loaded-leg speed keys. These are the professor's "input variables" (task order, AGV
# assignment, travelling speed) in the exact representation the optimiser searches over.
FEATURE_FAMILIES: tuple[str, ...] = ("sequence", "assignment", "empty_speed", "loaded_speed")
OBJECTIVES: tuple[str, ...] = ("makespan", "energy")
# Partition used for the Claim-3 failure-boundary read-out: the *kinematic* (continuous,
# per-leg speed) region vs the *topological* (sequencing / AGV-assignment) region. The
# exact Sobol landscape shows both objectives are topology-dominated -- the region where a
# flat, additive tabular attributor is expected to break down.
...
def critical_path_landscape(
    ...
    """
    # Imported lazily: faithfulness.py transitively imports Torch (the surrogate), which
    # the Sobol/Pareto core deliberately avoids.
...
# --------------------------------------------------------------------------------------
# Pareto-vs-dominated contrast (why are solutions Pareto-optimal?)
# --------------------------------------------------------------------------------------
@dataclass(frozen=True, slots=True)
...
# --------------------------------------------------------------------------------------
# Claim 3: the topological failure boundary of tabular TreeSHAP
# --------------------------------------------------------------------------------------
...
def tabular_failure_boundary(
    ...
    *,
    ...
    """Compare TreeSHAP's family-importance profile to the exact Sobol profile (Claim 3).
...
class LandscapeResult:
    """Full landscape analysis for one instance (the Req 2 deliverable)."""
...
def to_json_dict(result: LandscapeResult) -> dict[str, object]:
    """JSON-serialisable view of a :class:`LandscapeResult`."""
    return {
