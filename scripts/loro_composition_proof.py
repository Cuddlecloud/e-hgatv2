"""R4 proof-of-amortization on a COMPOSITION-DIVERSE instance set.

The real benchmark is transport-saturated (transport_frac ~0.88 everywhere), so the
amortized predictor cannot beat a constant there -- not because it can't learn, but
because there is nothing to learn. This script tests the predictor on a synthetic set
deliberately spanning the full composition spectrum (QC-bound .. transport-bound, via
num_agvs/num_qcs), where a constant baseline is weak. If the predictor recovers
composition from structural features and beats the constant baseline, R4 amortization
is demonstrated.

Leave-one-instance-out. Reports, per held-out instance and in aggregate:
  - model MAE vs naive constant-mean MAE (does the model beat a constant?)
  - between-instance composition recovery: corr( predicted instance-mean,
    true instance-mean ) across held-out instances (does structure -> composition?)
