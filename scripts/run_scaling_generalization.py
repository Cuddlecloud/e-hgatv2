...
- ``fused_r2_makespan`` / ``fused_r2_energy`` -- physics-fused head. Expected to HOLD: it
  predicts per-leg times (local, size-invariant) and the tropical max-plus DP aggregates
  them into a correct-magnitude makespan at ANY N; energy is exact-additive.
- ``tape_leg_jaccard`` -- fused critical path vs the exact O(N) TAPE oracle. Expected ~1
  (faithful by construction).
- ``attn_rho`` / ``attn_prec1`` -- attention vs the true per-task makespan sensitivity.
  Expected ~0 / ~chance (the dissociation: attention guides search but is not faithful).
...
def _train_curriculum(train_ns, seed, peak_power, core_samples, core_epochs, fused_samples,
    ...
    Fixes the single-N head's inability to see how the (coupled) power-contention density grows
    with N: pooling samples across ``train_ns`` exposes a range of contention regimes, so the
