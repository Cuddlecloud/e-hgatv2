"""scripts/run_aos_stats.py -- Module 5c entrypoint (Claim 2 statistics).
...
def _print(stats: AblationStats) -> None:
    console.rule("[bold white]AOS Ablation Statistics (Claim 2)")
    ...
    if hv is not None:
        ...
        if att is not None:
            ...
            msg = (
                f"\n[bold green]Claim 2 (HV):[/bold green] attention > random "
...
def main(args: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="AOS ablation statistics (Claim 2)")
