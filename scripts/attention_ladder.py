...
In NLP the "is attention explanation?" debate is unfalsifiable: there is no ground
truth. Here the problem is physically grounded -- the makespan is an exact max-plus
longest path, so every schedule has a *known* critical path and a *known* per-task
marginal makespan-sensitivity. We can therefore measure, at several granularities,
how well the surrogate's attention (vs TAPE's exact tropical gradient) aligns with the
objective's own structure:

  L1 coarse  precision@1 : is the single highest-signal task on the critical path?
  L2 medium  ROC-AUC     : does the signal rank on-critical-path tasks above off-path?
  L3 fine    Spearman rho: does the signal rank tasks by *graded* makespan-sensitivity?
  L4 effect  separation  : mean signal on critical tasks - mean on off-critical (>0?)

Hypothesis: attention degrades from L1->L3 -- it captures coarse objective-relevant
structure (which tasks matter, hence good search guidance, R3) but not the exact
bottleneck ranking (poor R2); TAPE stays high at every level. Both are compared to
the random baseline (precision@1 = critical-fraction; AUC = 0.5).
