...
Both the GNN (predicts the nonlinear waits) and TAPE (max-plus timing + composition) fire in
every unroll step, so neither is sidelined. Emits R2 / MAE per test-N + a compute-saved column
(exact-simulator wall time vs the model's batched forward).
