"""Is the guided-search benefit from attention's targeting, or from shared screening?

The attention and TAPE arms share the surrogate's offspring screening and differ only
in which task mutation targets. The granularity ladder shows attention does not localise
the critical path (AUC~=0.5), yet it guides as well as TAPE. This ablation isolates the
cause by adding the missing cell -- random mutation WITH screening:
...
If HV(A) ~= HV(R): mutation-targeting (hence attention's bottleneck signal) adds nothing
beyond screening -- the benefit is the shared objective-regression screening, exactly
why attention "guides well" despite carrying no critical-path information.
If HV(R) > HV(N): screening is the lever.
