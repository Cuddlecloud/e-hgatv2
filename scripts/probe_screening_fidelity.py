"""De-risk the surrogate-screening lever WITHOUT running a full multi-seed HV ablation.

Screening (attention_nsga2, screening_factor=k) generates k*P offspring, ranks them with
the fused surrogate, and spends exact evaluations only on the predicted-best P. Its entire
payoff hinges on ONE quantity: does `top-P-by-surrogate` recover the true-best-P *inside a
realistic near-Pareto offspring pool*? Global held-out R2 is an optimistic upper bound on
that (the pool is narrow; ranking there is harder). This probe measures it directly.
...
reports, per screening factor k in {2,4,8,16}:
  ...
  * HV-recovery  -- (HV_surrogate - HV_random) / (HV_oracle - HV_random), all TRUE HV:
        ~1.0 => screening picks as well as an oracle (strong lever)
        ~0.0 => screening no better than random selection (dead weight; skip the ablation)
        < 0  => screening is actively worse than random

All HVs are true (exact-evaluated) objectives against a common reference point, so the
comparison across k and across selection rules is apples-to-apples.
"""
