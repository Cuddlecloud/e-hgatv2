"""Pareto-front behaviour analysis via TAPE Pareto Tension Scores (PTS).

Addresses the requirement: *extract the explanation knowledge in a way that
characterises the Pareto front's behaviour*. For each instance we

1. train the core E-HGATv2 + the physics-fused TAPE head;
2. run the TAPE-guided NSGA-II to obtain the approximated Pareto set (schedules);
3. compute, for every Pareto point, the local trade-off weight ``lambda`` from the
   front tangent and the lambda-weighted TAPE attribution (PTS) per task;
4. quantify how the binding-bottleneck structure *migrates* along the front:
   - lambda range across the front (trade-off sweep actually covered),
   ...
   - PTS concentration = share of total PTS carried by the top-k tasks (sparsity =>
     interpretable, few tasks explain each point).

Writes experiments/fused_tape_guided/pts_frontier_<tag>.{json,md} and a combined
pts_frontier_summary.md.
"""
...
def analyse(spec: str, peak_power: float | None, args) -> dict:
    ...
    rows = pareto_tension_scores(points)  # sorted by makespan ascending

    # Per point: lambda + ranked tasks by pts_total.
    lambdas = [r["lambda"] for r in rows]
    top_tasks = []
    concentration = []
    for r in rows:
        tasks = sorted(r["tasks"], key=lambda t: t["pts_total"], reverse=True)
        ranked = [t["task"] for t in tasks]
        top_tasks.append(ranked)
        tot = sum(t["pts_total"] for t in r["tasks"]) or 1.0
        concentration.append(sum(t["pts_total"] for t in tasks[:TOPK]) / tot)
    ...
    result = {
        ...
        "pts_concentration_topk": float(np.mean(concentration)) if concentration else None,
        "topk": TOPK,
    ...
    print(f"  {spec}: front={result['n_front']} lambda[{result['lambda_min']:.2f},"
          ...
          f"conc@{TOPK}={result['pts_concentration_topk']:.2f}", flush=True)
    return result
...
def main() -> None:
    ...
    (OUT / f"pts_frontier_{tag}.json").write_text(json.dumps(results, indent=2))

    md = [f"# Pareto-front behaviour via TAPE PTS ({tag})\n",
          ...
          "(1 - Jaccard of top-3 at the two extremes), and the PTS concentration "
          "(share of total tension on the top-3 tasks)._\n",
          "| Instance | N | front | lambda range | top-3 (Cmax end) | top-3 (E end) | "
          "migration | PTS conc.@3 |",
          "|---|---|---|---|---|---|---|---|"]
    for r in results:
        md.append(
            ...
            f"{r['bottleneck_migration']:.2f} | {r['pts_concentration_topk']:.2f} |")
    (OUT / f"pts_frontier_{tag}.md").write_text("\n".join(md) + "\n")
    print("\n".join(md))
