...
This shares NOTHING with evaluator.py's composition: it does not track AGV-free times or
handoffs; it simply enforces each MILP inequality `var >= rhs` repeatedly until nothing
changes (Bellman-Ford on the DAG, which converges in <= N sweeps). The leg time/energy
kinematics are taken from physics.py (already validated against the book-chapter curves),
so the ONLY thing under test here is the timing composition (the max-plus structure).
...
def milp_timing(schedule, instance) -> tuple[float, float]:
    ...
    # Leg kinematics (same inputs the evaluator sees; only the COMPOSITION differs).
...
def milp_timing_lp(schedule, instance) -> float:
    ...
    C_max >= 0 and minimizes C_max. GLOP is a trusted simplex solver, so this is a
