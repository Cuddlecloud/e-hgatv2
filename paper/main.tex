...
underlying assignment--sequencing problem is NP-hard~\cite{homayouni2023}; exact MILP is
...
\paragraph{Then why does attention guide search as well as TAPE?}
Because the guidance benefit is not attention's. The attention and TAPE arms share the
surrogate's offspring \emph{screening} and differ only in which task mutation targets. An
ablation that adds the missing control---random mutation \emph{with} screening---isolates the
cause (Table~\ref{tab:ladder}, bottom): replacing attention-targeted by random-targeted
mutation (both screened) shifts $\mathrm{HV}/\mathrm{HV}^\star$ by only $-0.009$ (mixed sign,
within noise), whereas adding screening to random mutation improves it by $+0.042$
(consistent across instances, growing with $N$). The lever is the surrogate's objective
\emph{regression} used for screening---shared by both guided arms---not the attention
distribution. This reconciles the two facts: attention guides optimisation as well as TAPE
(it rides the same screening) while encoding none of the bottleneck structure TAPE recovers.
The network attains $R^2\!\approx\!0.99$ objective regression with attention that is, by every
granularity measurable against ground truth, indistinguishable from random---accurate
prediction does not imply an interpretable attention map.
...
ablation (HV/HV$^\star$, mean over instances) showing the optimisation benefit comes from the
shared surrogate \emph{screening}, not from attention-targeted mutation.}
\label{tab:ladder}
...
\midrule
\multicolumn{4}{l}{\emph{Search-guidance ablation $\;\Delta(\mathrm{HV}/\mathrm{HV}^\star)$:}}\\
attention-targeted $-$ random-targeted mutation (both screened) & \multicolumn{3}{c}{$-0.009$ \;(within noise)} \\
screening added to random mutation & \multicolumn{3}{c}{$\mathbf{+0.042}$ \;(consistent)} \\
\bottomrule
\end{tabular}
\end{table}
...
hypervolume (Holm-adjusted $p = 0.067$; attention is numerically ahead on the convergence
indicators). We therefore make the honest, defensible claim: \emph{routing the guidance
through the faithful TAPE signal preserves the optimisation performance of the unfaithful
attention signal.} Faithfulness comes at no cost in solution quality, and the same object
that steers the search is the one that correctly explains it. We do \emph{not} claim that
faithful guidance converges faster than attention---the data do not support that.
...
held-out $R^2$ falls to $\approx0.80$ (from $0.99$ uncoupled) and TAPE's Jaccard drops to
$0.87$ at $N=20$. Both graph-guided methods still beat all three non-learned baselines, but
the coupled claim is the weakest and is the clearest target for more training budget (more
samples, epochs and physics-unrolling steps for the power-wait head).
