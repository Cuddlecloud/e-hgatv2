...
Jaccard $0.85$--$1.00$ against an exact oracle, whereas the conventional learned
...
fine ranking, are no better than random at \emph{every} granularity---a verdict the exact
critical path makes directly measurable here. Routing the same faithful signal back into an NSGA-II search makes the
guided algorithm dominate mp-BRKGA, single-population BRKGA and an unguided control on
hypervolume in $11/11$ instances (Friedman $\chi^2_4=35.1$, $p=4.5\times10^{-7}$; all
post-hoc Holm-adjusted $p\le0.004$). Finally, aggregating TAPE over the approximated Pareto
...
\paragraph{What ``exact'' does and does not mean.}
...
DP \emph{exactly}---it is that DP's own subgradient, not a post-hoc saliency heuristic. The
...
residual leg error (leg-critical Jaccard $0.87$--$1.00$ against the exact simulator oracle);
...
\emph{relative to the model being explained}. Far from weakening the thesis, this is precisely
what makes faithfulness \emph{decidable} in this model class: because an exact attribution of
the surrogate exists for free, any residual gap between attention and the critical path is a
...
\label{sec:ladder}
In natural-language settings the ``is attention explanation?'' debate rests on \emph{proxy}
ground truth---erasure, gradient, and adversarial-attention tests---which is why it has
...
it directly. We train the core $+$ physics-fused head \emph{once} at $N{=}16$ and then, by
...
\begin{table}[t]
\centering\small
\caption{Size generalisation of the uncoupled surrogate: trained \emph{once} at $N{=}16$ and
...
global-readout core collapses; TAPE faithfulness (leg-Jaccard vs.\ the exact oracle) and the
...
hypervolume: across all $11$ instances at $20$ seeds the Holm-adjusted TAPE-vs-attention
$p = 0.83$ (Cliff's $\delta = +0.01$, negligible; TAPE ahead on $6/11$)---a genuine tie
rather than an underpowered one---while both graph-guided variants keep crushing the
classical baselines (TAPE $>$ mp-BRKGA on $11/11$, Holm $p<0.01$; Friedman
$p\approx4.5\times10^{-7}$). We therefore make the honest, defensible claim: \emph{routing the
...
is \emph{not} significant ($p=0.83$)---they are statistically tied, as Section~\ref{sec:ladder}
...
($\delta=+0.01$)---exactly the ``faithfulness for free'' reading: TAPE matches attention
while strictly dominating every non-learned baseline by a medium-to-large margin.
