...
Its input is the schedule itself, a heterogeneous graph of tasks, vehicles, cranes and
precedence relations, rather than a fixed-length vector of designed features; the model is
consequently size-agnostic, and one fitted at $N=16$ is evaluated unchanged up to $N=5000$.
...
distinct from the surrogate, and is the sole component fitted on designed features; the
target is one aggregate statistic per instance, for which a model of this size is
appropriate at the sample sizes available. Its training targets are supplied by the
surrogate and TAPE, which produce the fronts and their critical-path compositions.
Evaluation is by
