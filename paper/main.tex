...
This advantage is not constant---it \emph{amplifies with scale}. Table~\ref{tab:optladder}
sweeps $N\in\{10,20,40,80,160\}$ in both regimes with every baseline present, at $20$ seeds
...
it. And the one margin that does not separate---uncoupled $N{=}40$ against mp-BRKGA
($+0.022$; $+0.123$ coupled, the two smallest in the table)---is a tie rather than a loss,
and it reverses by $N{=}80$. It coincides with a discontinuity in the instance generator
rather than with anything in the model: \texttt{scaled\_fleet} allocates $\lceil N/12
\rfloor$ AGVs and $3$ QCs, making $N{=}40$ both the most AGV-contended point on the ladder
($13.3$ tasks per AGV, the ladder maximum) and the crossover at which the binding resource
flips from the cranes to transport (AGV/QC $=1.0$). The guided arms earn their margin by
exploiting a clear bottleneck, so it is smallest exactly where the bottleneck is most
ambiguous, and returns once the instances are transport-bound---a property of the instance
family, not a size limit of the surrogate.
