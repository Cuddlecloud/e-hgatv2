...
This recurrence is the critical path method of Kelley and Walker~\cite{kelley1959cpm}: the
forward pass is CPM's earliest-start computation and the backward trace of argmax
predecessors is CPM's critical-path recovery. Given the leg durations of a resolved
schedule, CPM returns the critical path exactly, at negligible cost, and no learned model
improves on it. What CPM cannot do is supply the durations. Section~\ref{sec:fused}
therefore embeds the same recurrence as a differentiable layer over \emph{predicted}
durations, which is what makes the path available for schedules the exact evaluator has not
been run on, and makes it a gradient that trains the network that produced them.
...
\paragraph{Relation to CPM and to differentiable dynamic programming.} Equation~\eqref{eq:tape}
restates the slack property of the critical path method~\cite{kelley1959cpm}, and the
backward routing of the proof is CPM's critical-path trace. On a schedule already passed
through the exact evaluator the leg durations are known, CPM returns $\pi^\star$ exactly,
and TAPE returns the same set: the two coincide, and the learned model contributes nothing.
Differentiating through a dynamic program is likewise
established~\cite{mensch2018ddp,rush2020torchstruct}, though that line smooths the $\max$ to
obtain a dense Jacobian, where the hard $\argmax$ retained here is what keeps the
attribution a binary path indicator. What the present construction adds is the pairing:
the durations are supplied by the graph network, so $\pi^\star$ is obtained for schedules
the evaluator has \emph{not} been run on, and the same indicator is the gradient by which
those durations are learned~\eqref{eq:tapechain}. The novelty claimed is therefore not the
attribution rule but its availability off the evaluated set, and the exact reference it
provides for auditing the attention weights (Section~\ref{sec:results}).
...
$\mathrm{HV}^\star$ and sweeps $N\in\{10,20,40,80,160\}$ in both regimes. Intuitively, at
fixed budget the combinatorial volume $|\mathcal Z|=A^N N!\,V^{2N}$ explodes, so an unguided
search cannot cover it; the surrogate's critical-path signal concentrates the same budget on
the binding decisions, and that concentration matters more the larger the space. The
convergence trace of Figure~\ref{fig:conv} is a single point of that sweep ($N{=}50$,
$100$ generations) against single-population BRKGA; the full ladder below carries every
baseline, including mp-BRKGA, at $20$ seeds per cell.

\paragraph{Ladder to $N{=}160$ over both regimes with all baselines.}
Table~\ref{tab:optladder} extends the sweep to $N\in\{10,20,40,80,160\}$ with every baseline
present, including mp-BRKGA, and with the peak-power-coupled regime run alongside the
uncoupled one: $20$ seeds per cell, $40$ generations, matched exact-evaluation budget,
population $P=5N$, offspring screening factor $k=4$. Margins are quoted for the TAPE-guided
arm, fixed in advance; the attention-guided arm is reported beside it and the two overlap
within their intervals at every size, so this ladder does not separate them.

Three readings. \emph{(i)} \textbf{Every cell is a win.} TAPE-guided search leads all four
baselines at all five sizes in both regimes, and in $29$ of the $30$ comparisons the margin
exceeds the combined $95\%$ confidence interval; the single exception is uncoupled $N{=}40$
against mp-BRKGA ($+0.022$), which is a tie rather than a loss. \emph{(ii)} \textbf{The
advantage widens with $N$.} Against the unguided control the TAPE margin runs
$+0.101\!\to\!+0.584$ uncoupled and $+0.107\!\to\!+0.674$ coupled; against mp-BRKGA,
$+0.173\!\to\!+0.557$ and $+0.140\!\to\!+0.615$. The mechanism is that the classical
baselines collapse at the top of the ladder while the guided arms do not: at $N{=}160$
uncoupled, single-population BRKGA reaches $0.003$, the unguided control $0.012$ and
mp-BRKGA $0.040$ of $\mathrm{HV}^\star$, against $0.597$ for TAPE. At a fixed budget of $40$
generations, a search without a surrogate has effectively not started at this size.
\emph{(iii)} \textbf{Absolute quality still falls with $N$ uncoupled} ($0.944\!\to\!0.597$),
so the widening margin is relative, not an absolute gain; under coupling it does not fall
($0.871\!\to\!0.879$), because the power budget contracts the reachable objective space and
the reference front with it. We report the dominance as relative to the baselines at matched
budget, which is what the ladder measures.

The narrowing at $N{=}40$ is visible in both regimes ($+0.022$ and $+0.123$ against
mp-BRKGA, the two smallest margins in the table) and then reverses. It coincides with a
discontinuity in the instance generator's fleet provisioning rather than with anything in
the model: \texttt{scaled\_fleet} allocates $\lceil N/12 \rfloor$ AGVs and $3$ QCs, so
$N{=}40$ is simultaneously the most AGV-contended point on the ladder ($13.3$ tasks per AGV,
the ladder maximum) and the crossover at which the binding resource flips from the cranes to
transport (AGV/QC $=1.0$). The guided arms earn their margin by exploiting a clear
bottleneck, so the margin is smallest exactly where the bottleneck is most ambiguous, and it
returns at $N{=}80$ and $N{=}160$ once the instances are transport-bound. The effect is a
property of the instance family, not a size limit of the surrogate.
...
\addplot[fill=blue!55] coordinates {(10,0.944)(20,0.828)(40,0.846)(80,0.574)(160,0.597)};
\addplot[fill=blue!20] coordinates {(10,0.929)(20,0.804)(40,0.870)(80,0.617)(160,0.678)};
\addplot[fill=orange!60] coordinates {(10,0.771)(20,0.630)(40,0.824)(80,0.341)(160,0.040)};
\addplot[fill=red!55] coordinates {(10,0.800)(20,0.624)(40,0.516)(80,0.071)(160,0.003)};
\addplot[fill=black!35] coordinates {(10,0.843)(20,0.608)(40,0.578)(80,0.149)(160,0.012)};
...
\addplot[fill=blue!55] coordinates {(10,0.871)(20,0.826)(40,0.876)(80,0.823)(160,0.879)};
\addplot[fill=blue!20] coordinates {(10,0.864)(20,0.812)(40,0.869)(80,0.788)(160,0.818)};
\addplot[fill=orange!60] coordinates {(10,0.732)(20,0.635)(40,0.752)(80,0.500)(160,0.264)};
\addplot[fill=red!55] coordinates {(10,0.756)(20,0.574)(40,0.432)(80,0.285)(160,0.175)};
\addplot[fill=black!35] coordinates {(10,0.765)(20,0.462)(40,0.440)(80,0.338)(160,0.205)};
\end{axis}
\end{tikzpicture}
\caption{Scaling of solution quality, $20$ seeds per cell at matched exact-evaluation budget
(same data as Table~\ref{tab:optladder}). Both graph-guided arms lead every baseline at every
size in both regimes, and the margin widens with $N$: by $N{=}160$ the classical baselines
have effectively not converged within the $40$-generation budget (uncoupled: sp-BRKGA
$0.003$, random $0.012$, mp-BRKGA $0.040$, against $0.597$ for TAPE). The narrowing at
$N{=}40$ tracks a discontinuity in fleet provisioning, discussed in the text.}
\label{fig:scale}
\end{figure}
...
($R^2\!\approx\!0.99$, Section~\ref{sec:results}) is unavailable to it by construction.
