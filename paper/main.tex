...
\begin{enumerate}[leftmargin=*,label=\textbf{R\arabic*.},ref=R\arabic*,nosep]
  \item\label{req:importance} Identify, by post-hoc analysis of solved instances, the
    quantities that drive the objectives.
  \item\label{req:landscape} Account for the Pareto-optimality of a given solution in
    terms of the input variables: task order, vehicle speed and crane workload.
  \item\label{req:guidance} Feed knowledge gathered during the search back to the
    optimiser, selecting which neighbourhood to explore and which operator to apply.
  \item\label{req:front} Extract that knowledge in a form that characterises the Pareto
    front rather than a single schedule.
\end{enumerate}
...
signal over the input variables (\ref{req:importance}, \ref{req:landscape}), as a steering
signal fed back to NSGA-II (\ref{req:guidance}), and, aggregated over the front with a
trade-off weight, as a description of front behaviour (\ref{req:front}).
...
heterogeneous graph surrogate---serves simultaneously as a post-hoc explanation
(\ref{req:importance}, \ref{req:landscape}), a search controller (\ref{req:guidance}), and a
description of Pareto-front behaviour (\ref{req:front}) for bi-objective quay-crane and
