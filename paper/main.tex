...
Nor does it rest on a designed feature vector. The surrogate reads the schedule itself, as
a heterogeneous graph of tasks, vehicles, cranes and precedence relations, so the quantities
that drive the objectives are located in that structure rather than summarised into
covariates chosen beforehand. One consequence is that the same trained model applies to
instances of any size: a model fitted at $N=16$ is evaluated unchanged up to $N=5000$,
where a surrogate over a fixed-length feature vector fitted at one size cannot be evaluated
at another at all.
...
features are $N$, the AGV and QC counts, and the mean QC handling time. This predictor is
separate from the surrogate and is the one place a fixed feature vector is used: the target
is a single aggregate statistic per instance, and with a $20$-instance training set a model
of that size is the appropriate one. The surrogate and TAPE supply its inputs, producing
the fronts and the critical-path compositions it is fitted to. Evaluation is by
