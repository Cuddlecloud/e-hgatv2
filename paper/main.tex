...
Four requirements on such a solver are treated together here. The first is to identify,
by post-hoc analysis of solved instances, the quantities that drive the objectives. The
second is to account for the Pareto-optimality of a given solution in terms of the input
variables: task order, vehicle speed and crane workload. The third is to feed knowledge
gathered during the search back to the optimiser, selecting which neighbourhood to
explore and which operator to apply. The fourth is to extract that knowledge in a form
that characterises the Pareto front rather than a single schedule.
...
The second requirement is a landscape one: to determine which input variables (task dispatch
order, AGV travelling speed, crane workload) fix the objective values, and thereby why
specific solutions are Pareto-optimal. TAPE resolves this exactly. For any schedule
