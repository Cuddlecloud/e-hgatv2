...
\author{Aayush Jha\\[2pt]
\small Code and data: \url{https://github.com/aayushjha1729/e-hgatv2}}
\date{\today}
...
true critical path with Jaccard $0.85$--$1.00$ against an exact oracle, whereas the
...
validated against the exact oracle---the coupled constraint is introduced in this work: it does
...
enough for an exact brute-force Pareto oracle. Their generator is specified in full in
...
Section~\ref{sec:fidelity} is the aggregate of that last check. The source code, instances,
seeds and result artifacts are available at
\url{https://github.com/aayushjha1729/e-hgatv2}.
...
\paragraph{Environment.} Python 3.12.11, PyTorch 2.6.0, PyTorch Geometric 2.8.0. The
surrogate is executed on CPU for all reported results: the scatter-max aggregation and the
$(\max,+)$ relaxation are order-sensitive under non-deterministic GPU reductions, and CPU
execution makes every number bit-reproducible from its recorded seed. Search experiments
were sharded across CPU cores, one shard per (instance, seed-block) pair.
...
the code computes and are checkable only against it. The source tree described here is
available at \url{https://github.com/aayushjha1729/e-hgatv2}, together with the instance
...
evaluator, oracle and BRKGA baselines depend only on \texttt{numpy} and \texttt{scipy} and
...
\midrule
\texttt{environment} & no  & Instance construction, distance matrix, published speed/power constants, random-key decoder, exact evaluator (both regimes), brute-force Pareto oracle \\
