...
\begin{enumerate}[leftmargin=*,label=\textbf{R\arabic*.},ref=R\arabic*,nosep]
  \item\label{req:importance} \textbf{Driving quantities.} Identify the quantities that
    actually drive the objectives, without fixing them in advance by hand.
...
longest path embedded inside a learned surrogate. The signal is the dynamic program's own
subgradient, computed in the same backward pass that trains the network and consumed by the
optimiser while it runs.
The surrogate operates on the schedule as a heterogeneous graph of tasks, vehicles, cranes
and precedence relations, and is therefore size-agnostic: one fitted at $N=16$ is evaluated
unchanged up to $N=5000$.
...
Faithfulness is used throughout in a specific sense: the signal identifies the operations
that set the makespan. Learned attention weights read directly as explanations do not meet
this criterion here, and Section~\ref{sec:results} quantifies the gap.
...
features are $N$, the AGV and QC counts, and the mean QC handling time. The predictor is a
separate model from the surrogate; its target is a single aggregate statistic per instance,
and its training targets are the fronts and critical-path compositions produced by the
surrogate and TAPE. Evaluation is by
