...
\begin{enumerate}[leftmargin=*,label=\textbf{R\arabic*.},ref=R\arabic*,nosep]
  \item\label{req:importance} \textbf{Feature engineering.} Derive, from solved instances,
    the quantities that actually drive the objectives.
  \item\label{req:landscape} \textbf{Landscape and feature importance.} Account for the
    Pareto-optimality of a given solution in terms of the input variables: task order,
    vehicle speed and crane workload.
  \item\label{req:guidance} \textbf{Feedback to the optimiser.} Use the knowledge gathered
    during the search to steer it, selecting which neighbourhood to explore and which
    operator to apply.
  \item\label{req:front} \textbf{Front behaviour.} Extract that knowledge in a form that
    characterises the Pareto front itself rather than a single schedule.
\end{enumerate}

A single mathematical object addresses all four: the gradient of an exact max-plus
longest path embedded inside a learned surrogate. It is not an explainer fitted to a
trained model after the fact but the dynamic program's own subgradient, and it serves as an
importance signal over the input variables of a solved schedule (\ref{req:importance},
\ref{req:landscape}), as a steering signal fed back to NSGA-II (\ref{req:guidance}), and,
...
heterogeneous graph surrogate---serves simultaneously as an explanation of a solved
schedule (\ref{req:importance}, \ref{req:landscape}), a search controller
(\ref{req:guidance}), and a description of Pareto-front behaviour (\ref{req:front}) for
bi-objective quay-crane and SA-AGV scheduling. It is provably faithful where learned attention is not, and routing the
