...
Attention's precision@1 ($0.67$--$0.80$) looks superficially acceptable only because, with
few tasks, naming any one of a handful of critical tasks is likely by chance (the random
baseline is $\approx 0.05$--$0.10$); the near-zero rank correlation exposes that the full
ranking is noise. This is a clean, quantified instance of the ``attention is not
explanation'' phenomenon~\cite{jain2019attention} on a real scheduling problem, and it is
the justification for routing guidance through TAPE rather than attention.
