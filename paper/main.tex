...
\subsection{The peak-power-coupled regime (a synthetic stress-test)}
\label{sec:coupled}
...
\emph{and} starting it keeps the sum of concurrently-running leg powers $\le P_{\max}$;
...
\paragraph{Scope and honesty.} Two caveats we state plainly. \emph{(i)} The power arbitration
is a \emph{fixed dispatch policy} (the priority rule above), not a decision variable; the
regime optimises the assignment, order and speeds \emph{under} that policy. \emph{(ii)} Unlike
the uncoupled objective---the published recurrence~\cite{fontes2023flowtime}, validated
against the exact oracle---this coupled constraint is \emph{our own synthetic construction}:
it is \emph{not} part of the FSMJ/book-chapter benchmark and we make \emph{no} claim that it
models a specific real installation. Its role is purely methodological: it is the regime where
the makespan has no closed form, ``which leg binds'' is non-obvious, and a learned
\emph{differentiable} surrogate with exact gradients (TAPE) earns its place---which is exactly
why the fused head carries an explicit power-wait term (Section~\ref{sec:fused}).

\section{Method}
\label{sec:method}

\paragraph{The method in one paragraph.} There is no labelled training corpus. An exact
...
\paragraph{The exact evaluator (the oracle).} Given a decoded schedule $s(z)$, the evaluator
returns the objective pair $f(s)=(\Cmax,E)$ \emph{and} the per-task physical quantities the
...
the ground-truth oracle against which we measure surrogate fidelity
...
\emph{only} by these labels: (i) the E-HGATv2 core and its scalar read-out
...
model; its tropical gradient both \emph{explains} a schedule (R2/R4) and \emph{steers} the
next generation's mutation and offspring screening (R3); the steered search then visits better
schedules. The surrogate is trained \emph{once} per instance on the sampled set; on the
...
\EndFor
\State \Return Pareto set, with per-front TAPE attributions and Pareto Tension Scores (R4)
\end{algorithmic}
\end{algorithm}
...
reference front $\mathrm{PF}^\star$ (the exact oracle for \textsf{SD-5}; elsewhere the
...
\paragraph{Protocol.} Each cell is a mean over $20$ seeds (\textsf{SD-20-C}: $5$) with a $95\%$
...
evaluator doubles as a built-in oracle for validation: a regression suite checks that the
...
Section~\ref{sec:fidelity} is the aggregate of that last check. Code, instances, and seeds are
...
edge on HV (Table~\ref{tab:stats}, ranks $1.27$ vs.\ $1.73$, $p_{\text{Holm}}=0.175$):
...
rejects equality of the five methods with $\chi^2_4 = 37.2$, $p = 1.6\times10^{-7}$; the
...
\paragraph{Significance across instances.} The headline runs use $S{=}20$ seeds (the coupled
\textsf{SD-20-C}, by far the most expensive, uses $5$). The Dem\v{s}ar protocol aggregates the
comparison over the $11$ instances (where TAPE vs.\ mp-BRKGA reaches $p_{\text{Holm}}=0.0029$);
...
mp-BRKGA the effect is \emph{large} ($\delta=+0.70$ on HV and $-0.62$ on GD$^+$), against
single-population BRKGA \emph{large} ($\delta=+0.52$) and against the random control
\emph{medium} ($\delta=+0.46$) on HV, and against attention it is \emph{negligible} and
slightly negative ($\delta=-0.12$)---exactly the ``faithfulness for free'' reading: TAPE
matches attention while strictly dominating every non-learned baseline by a medium-to-large
margin.
...
plot in Figure~\ref{fig:forest}). At $20$ seeds the $95\%$ CI \emph{excludes zero on $10$ of
$11$ instances}---a genuine per-instance significance criterion---failing only on the
$5$-seed coupled \textsf{SD-20-C}, whose wide interval reflects its seed count rather than a
vanishing gap (point estimate still $+0.079$). Moving from $5$ to $20$ seeds lifted this from
$8/11$ to $10/11$, exactly as expected.
...
($10^{4}$ resamples) $95\%$ CI on the per-seed gap (TAPE$-$mp), at $20$ seeds
(\textsf{SD-20-C}$^\dagger$: $5$). A CI excluding $0$ is per-instance significance and holds on
$10/11$ instances; only the $5$-seed coupled case fails (point estimate still $+0.079$).}
\label{tab:boot}
\setlength{\tabcolsep}{6pt}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lccccc}
...
\midrule
\textsf{SD-5}    & 0.973 & 0.928 & $[+0.032,+0.059]$ & 4 & yes\\
\textsf{SD-8}    & 0.954 & 0.906 & $[+0.033,+0.064]$ & 1 & yes\\
\textsf{SD-10}   & 1.008 & 0.923 & $[+0.054,+0.117]$ & 11 & yes\\
\textsf{SD-15}   & 0.919 & 0.807 & $[+0.078,+0.144]$ & 4 & yes\\
\textsf{SD-20}   & 0.907 & 0.815 & $[+0.039,+0.146]$ & 33 & yes\\
\textsf{L07}     & 0.973 & 0.929 & $[+0.028,+0.061]$ & 11 & yes\\
\textsf{L15}     & 0.904 & 0.761 & $[+0.106,+0.184]$ & 0 & yes\\
\textsf{L21}     & 1.017 & 0.853 & $[+0.116,+0.213]$ & 2 & yes\\
\textsf{L35}     & 0.970 & 0.913 & $[+0.041,+0.072]$ & 1 & yes\\
\textsf{SD-10-C} & 1.002 & 0.844 & $[+0.082,+0.234]$ & 22 & yes\\
\textsf{SD-20-C}$^\dagger$ & 0.846 & 0.767 & $[-0.049,+0.220]$ & 5 & no\\
\bottomrule
\end{tabular}}
\end{table}
...
\addplot[only marks, mark=*, mark size=1.6pt, blue,
  ...
  coordinates {
    (0.045,11) += (0.014,0) -= (0.013,0)
    (0.048,10) += (0.016,0) -= (0.015,0)
    (0.085,9)  += (0.032,0) -= (0.031,0)
    (0.112,8)  += (0.032,0) -= (0.034,0)
    (0.093,7)  += (0.053,0) -= (0.054,0)
    (0.044,6)  += (0.017,0) -= (0.016,0)
    (0.143,5)  += (0.041,0) -= (0.037,0)
    (0.164,4)  += (0.049,0) -= (0.048,0)
    (0.056,3)  += (0.016,0) -= (0.015,0)
    (0.158,2)  += (0.076,0) -= (0.076,0)
...
zero on $10/11$ instances---failing only on the $5$-seed coupled \textsf{SD-20-C}---and never
excludes zero on the wrong side. This is the visual companion to Table~\ref{tab:boot}.}
\label{fig:forest}
\end{figure}
