# E-HGATv2 vs mp-BRKGA — aggregate benchmark result & what the statistics mean
...
average ranks on **every** Pareto-quality metric, and **mp-BRKGA ranks dead last on all three**.
The TAPE-vs-mp-BRKGA gap is statistically significant after Holm correction with **medium-to-large**
effect sizes. So the claim "we beat *even* mp-BRKGA" is supported **on the aggregate benchmark**.

> Scope caveat (see §4): this is the *fixed-size* benchmark. In the separate **N-scaling** study,
...
## 4. What each metric / statistic entails
...
honest bar for "we beat this baseline."
...
- **What is *not* claimed here:** that guided beats mp-BRKGA at *every* problem size. The N-scaling
  study (`PAPER_FINDINGS.md`, R3) shows mp-BRKGA is the one baseline that does not stall as N grows —
...
- **Mechanism honesty:** the GNN's contribution is primarily **screening** (surrogate prediction),
