"""Tests for the E-HGATv2 model (Module 3, ``surrogate/ehgatv2.py``).
...
def test_attention_is_non_degenerate() -> None:
    # The whole point: cross-resource attention must actually vary across arcs
    # (per-relation GATv2 softmax over a single predecessor would be a constant 1).
    graphs = _graphs()
    model = _model()
    spreads = []
    for g in graphs:
