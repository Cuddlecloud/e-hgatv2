"""Unit tests for fused-GNN TAPE guidance (Signal #3) wired into the NSGA-II search.

These verify the *wiring*, determinism, and signal shape -- not surrogate quality (an
untrained fused model suffices because the returned front is built from exact
``evaluate()`` objectives, so it is always physically valid regardless of the head).
"""
