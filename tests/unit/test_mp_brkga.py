...
- the multi-population scheme recovers the objective extremes at least as well as the
  single-population BRKGA (the paper's Fig. 6 claim that mp- dominates single-pop on the
  boundaries) -- the qualitative behaviour we must not regress.

The decisive *quantitative* gate (reproducing the paper's Table 2 GD+/Delta on the real DS
instances) requires the advisor's instance data and is tracked separately in the results plan.
"""
