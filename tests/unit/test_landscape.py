"""Unit tests for the exact landscape analysis (Module 6, Req 2).
...
physical sanity checks encode ground truth that any correct decomposition must satisfy
(e.g. energy variance is dominated by the speed families).
"""
...
def test_sobol_energy_is_routing_dominated(instance):
    """Physical truth (and a headline result): energy is dominated by *routing*, not speed.

    Loaded-leg distance is fixed per task, so the only speed lever on a fixed distance
    (loaded_speed) is second-order, and empty_speed merely modulates the empty leg by a few
    percent. What actually drives energy variance is *which* empty (repositioning) legs are
    travelled -- a topological function of sequence + AGV assignment. So the structural
    families must dominate the energy decomposition, contradicting the naive "energy is
    kinematic" intuition a flat attributor would encode.
    """
...
def test_sobol_makespan_is_structure_dominated(instance):
    ...
    Makespan is a max-plus longest path, so the routing/sequencing decisions that shape the
    critical path carry far more total-order variance than the per-leg speeds. This cascade
    coupling is exactly what a flat additive attributor cannot represent.
    """
    res = grouped_sobol(instance, base_samples=256, seed=0)
