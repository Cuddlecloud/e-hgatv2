...
def test_guidance_signal_matches_pergraph_coupled() -> None:
    """Coupled: batched and per-graph use the SAME coupled event DAG, so the guidance signal
    must match the per-graph reference task-for-task (same first-argmax tie-break)."""
